...
### Java 入门（基础概念与常识）
{: id="20210301203429-ux8lda5" updated="20210302194531"}

- {: id="20210302194541-sbgs4w9"}#### Java 语言特点
...
- {: id="20210302194715-ao3ijw1"}#### JVM JDK 和 JRE
      ...
      - {: id="20210302190031-jql6lni"}重点
        ...
        - {: id="20210302190625-ojyyb56"}热点代码
          ...
          - {: id="20210302190629-33wk82v" updated="20210302190633"}评估方法
            ...
            - {: id="20210302191453-e3u1u08"}((20210302001049-hpmqlu6 "JVM"))会根据代码每次被执行的情况收集信息并相应地做出一些优化
              {: id="20210302191453-j84iq9p" updated="20210302192826"}
        ...
        - {: id="20210302192729-vsp2xyk"}JIT编译器
          ...
          - {: id="20210302192720-0eh3xac"}特点
            ...
            - {: id="20210302190326-5tvl6g2"}当 JIT 编译器完成编译后，其会将((20210302190625-ojyyb56 "热点代码"))对应的机器码保存下来，下次可以直接使用
              {: id="20210302190326-mkm2tdc" updated="20210302194031"}
        ...
        - {: id="20210302193113-4x815se"}AOT(Ahead of Time Compilation)
          ...
          - {: id="20210302193043-8os1ief"}定义
            ...
            - {: id="20210302193053-25ocl3w"}JDK 9引入的一种新的编译模式
              {: id="20210302193053-xr5v7la" updated="20210302193105"}
          ...
          - {: id="20210302193214-vr8crp0"}特点
            ...
            - {: id="20210302193220-bplpwh8"}编译质量是比不上 JIT 编译器
              {: id="20210302193220-jbxb1ef" updated="20210302193737"}
            {: id="20210302193825-3hekhzq"}
          - {: id="20210302193824-qvv7hd4"}JDK 支持分层编译和 AOT 协作使用
            {: id="20210302193824-j29n8ow" updated="20210302194414"}
          {: id="20210302194906-l42hpen"}
  ...
  - {: id="20210302194904-i48ne1j"}JDK 和 JRE
    ...
    - {: id="20210302195245-jbqu5im"}JDK（Java Development Kit）
      ...
      - {: id="20210302195402-u3c0scy"}定义
        {: id="20210302195402-ctpok4r" updated="20210302195434"}
    ...
    - {: id="20210302195454-efjfu07"}JRE
      {: id="20210302195454-jikyva9" updated="20210302195501"}

      - {: id="20210302195502-izpua7r"}定义
        {: id="20210302195502-ijhxk2c" updated="20210302195506"}

        - {: id="20210302195508-pur4bj8"}Java 运行时环境
          {: id="20210302195508-r91isg0" updated="20210302195515"}
      ...
      - {: id="20210302195518-mzeemi4"}包括
        ...
        - {: id="20210302195521-73ln97d"}Java 虚拟机（JVM）
          {: id="20210302195521-lf8ni8g" updated="20210302195525"}
        - {: id="20210302195526-hzsbb16"}Java 类库
          {: id="20210302195526-zmmb2qw" updated="20210302195530"}
        - {: id="20210302195530-91y9fjm"}java 命令和其他的一些基础构件。但是，它不能用于创建新程序。
          {: id="20210302195530-oregfop" updated="20210302195530"}
        {: id="20210302195522-jmm1bm9"}
      {: id="20210302195502-kslo3q3"}
    {: id="20210302195246-mm47y44"}
  {: id="20210302194722-18llue5"}
{: id="20210301220753-apem785" updated="20210302194541"}
...
3. {: id="20210301203429-acpklbu"}占内存大小 字符常量只占 2个字节; 字符串常量占若干个字节 (**注意： char 在 Java 中占两个字节**),
   {: id="20210301203429-zxno64k"}

   > 字符封装类 `Character` 有一个成员常量 `Character.SIZE` 值为 1,单位是`bits`,该值除以 8(`1byte=8bits`)后就可以得到 2个字节
...
> 补充（来自[issue 75](https://github.com/Snailclimb/JavaGuide/issues/675)）：在 Java 9之后，String 、`StringBuilder` 与 `StringBuffer` 的实现改用 byte 数组存储字符串 `private final byte[] value`
...
使用 Java 7之后的 `try-with-resources` 语句改造上面的代码:
...
Java 线程在运行的生命周期中的指定时刻只可能处于下面  种不同状态的其中一个状态（图源《Java 并发编程艺术》4节）。
...
Java Io 流共涉及 40 多个类，这些类看上去很杂乱，但实际上很有规则，而且彼此之间存在非常紧密的联系， Java I0 流的 40 多个类都是从如下 4个抽象类基类中派生出来的。
...
- {: id="20210301203429-ham8h04"}**NIO (Non-blocking/New I/O):** NIO 是一种同步非阻塞的 I/O 模型，在 Java 4中引入了 NIO 框架，对应 java.nio 包，提供了 Channel , Selector，Buffer 等抽象。NIO 中的 N 可以理解为 Non-blocking，不单纯是 New。它支持面向缓冲的，基于通道的 I/O 操作方法。 NIO 提供了与传统 BIO 模型中的 `Socket` 和 `ServerSocket` 相对应的 `SocketChannel` 和 `ServerSocketChannel` 两种不同的套接字通道实现,两种通道都支持阻塞和非阻塞两种模式。阻塞模式使用就像传统中的支持一样，比较简单，但是性能和可靠性都不好；非阻塞模式正好与之相反。对于低负载、低并发的应用程序，可以使用同步阻塞 I/O 来提升开发速率和更好的维护性；对于高负载、高并发的（网络）应用，应使用 NIO 的非阻塞模式来开发
  {: id="20210301203429-e2z43e6"}
- {: id="20210301203429-h6kcktr"}**AIO (Asynchronous I/O):** AIO 也就是 NIO 2。在 Java 7中引入了 NIO 的改进版 NIO 2,它是异步非阻塞的 IO 模型。异步 IO 是基于事件和回调机制实现的，也就是应用操作之后会直接返回，不会堵塞在那里，当后台处理完成，操作系统会通知相应的线程进行后续的操作。AIO 是异步 IO 的缩写，虽然 NIO 在网络操作中，提供了非阻塞的方法，但是 NIO 的 IO 行为还是同步的。对于 NIO 来说，我们的业务线程是在 IO 操作准备好时，得到通知，接着就由这个线程自行进行 IO 操作，IO 操作本身是同步的。查阅网上相关资料，我发现就目前来说 AIO 的应用还不是很广泛，Netty 之前也尝试使用过 AIO，不过又放弃了。
...
- {: id="20210301203429-9pakoxo"}https://stackoverflow.com/questions/190445/what-is-the-difference-between-jdk-and-jre
